...
uint32_t Crc32::update(uint32_t crc, const void* data, size_t len) {
  ...
  static const uint32_t table[256] = {
      ...
      0xDBBB9F86, 0xACBCF940, 0x32D86CE3, 0x45DF5C75, 0xDCD60DCF, 0xABD13D59,
...
uint64_t bytes_hash(const std::string& s, uint64_t seed) {
  return bytes_hash(s.data(), s.size(), seed);
}

uint64_t pair_hash(uint64_t a, uint64_t b) {
  uint64_t buf[2] = {a, b};
  return bytes_hash(buf, sizeof(buf));
}
